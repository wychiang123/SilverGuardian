...
function computeFinalResult(
  ...
  // Tiered blending: weight depends on how strong the rule signal is
  // ruleScore=0 → trust AI fully; weak rule → AI-dominant; strong rule → equal weight
  let finalScore: number;
  if (ruleScore === 0) {
    finalScore = ai.ai_score;
  } else if (ruleScore < 30) {
    finalScore = Math.round(ruleScore * 0.3 + ai.ai_score * 0.7);
  } else {
    finalScore = Math.round((ruleScore + ai.ai_score) / 2);
  }
